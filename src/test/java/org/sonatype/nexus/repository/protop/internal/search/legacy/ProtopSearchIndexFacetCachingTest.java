...
public class ProtopSearchIndexFacetCachingTest
    extends TestSupport
{
  static class ProtopSearchIndexFacetCachingImpl
      extends ProtopSearchIndexFacetCaching
  {
    @Inject
    public ProtopSearchIndexFacetCachingImpl(final EventManager eventManager, final AssetManager assetManager) {
  ...
  private ProtopSearchIndexFacetCaching underTest;
  ...
  public void setUp() throws Exception {
    when(repository.getName()).thenReturn("protop-test");
    when(repository.getFormat()).thenReturn(new ProtopFormat());
    ...
    }).getInstance(ProtopSearchIndexFacetCachingImpl.class);
    ...
    doReturn(ProtopFormat.NAME).when(asset).format();
  ...
  public void testSearchIndex_FrozenDatabase_CachedIndex() throws Exception {
    when(storageTx.findAssetWithProperty(MetadataNodeEntityAdapter.P_NAME, ProtopFacetUtils.REPOSITORY_ROOT_ASSET, bucket))
        .thenReturn(asset);
  ...
  public void testSearchIndex_FrozenDatabase_NoCachedIndex() throws Exception {
    when(storageTx.createAsset(bucket, new ProtopFormat())).thenReturn(asset);
    when(storageTx.createBlob(eq(ProtopFacetUtils.REPOSITORY_ROOT_ASSET), any(), any(), any(), any(), anyBoolean()))
  ...
  public void whenInvalidateCacheSearchIndex_Expect_Asset_Removed() {
    ...
    verify(eventManager, times(1)).post(any(ProtopSearchIndexInvalidatedEvent.class));
  }
}
