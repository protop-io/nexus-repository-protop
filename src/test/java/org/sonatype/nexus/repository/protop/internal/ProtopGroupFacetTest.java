...
import static org.sonatype.nexus.repository.protop.internal.ProtopHandlers.T_PACKAGE_NAME;
import static org.sonatype.nexus.repository.protop.internal.ProtopHandlers.T_PACKAGE_ORG;
import static org.sonatype.nexus.repository.protop.internal.ProtopJsonUtils.bytes;
import static org.sonatype.nexus.repository.protop.internal.ProtopMetadataUtils.VERSIONS;

public class ProtopGroupFacetTest extends TestSupport {
  ...
  public void setUp() throws Exception {
    ...
    when(request.getPath()).thenReturn("/simple/something");

    when(groupRepository.getName()).thenReturn(ProtopGroupFacetTest.class.getSimpleName() + "-group");
    when(groupRepository.getFormat()).thenReturn(new ProtopFormat());
    ...
    when(storageTx.createAsset(any(), any(ProtopFormat.class))).thenReturn(packageRootAsset);
  ...
  public void whenMultipleDifferentResponsesWithDisabledMergeUpdatePackageRoot() throws Exception {
    setupMergeDisabledProtopGroupFacet();
    underTest.attach(groupRepository);
    underTest.doInit(configuration);
  ...
  public void whenSingleOrUnScopedResultShouldNotMerge() {
    assertThat(underTest.shouldServeFirstResult(createRandomMaps(1)), equalTo(true));
    assertThat(underTest.shouldServeFirstResult(createRandomMaps(1)), equalTo(true));

    setupMergeDisabledProtopGroupFacet();
    assertThat(underTest.shouldServeFirstResult(createRandomMaps(1)), equalTo(true));
    assertThat(underTest.shouldServeFirstResult(createRandomMaps(2)), equalTo(false));
  }
  ...
  private void setupProtopGroupFacet() {
    underTest = spy(new ProtopGroupFacet(true, repositoryManager, constraintViolationFactory, new GroupType()));
  }

  private void setupMergeDisabledProtopGroupFacet() {
    underTest = spy(new ProtopGroupFacet(false, repositoryManager, constraintViolationFactory, new GroupType()));
  }
  ...
  private Content buildMergedPackageRoot(final Map<Repository, Response> responses) throws IOException {
    ...
    verify(storageTx).createAsset(any(), any(ProtopFormat.class));
  ...
  private NestedAttributesMap createSimplePackageRoot(final String version) {
    ...
    versionAttributes.set("org", "org");
    versionAttributes.child("dist").set("tarball", "http://example.com/path/org-package-" + version + ".tar.gz");

    return packageRoot;
  }

  private void assertMergedSimplePackageRoot(final Supplier<InputStream> supplier, final String... versions) throws IOException {
    NestedAttributesMap result = ProtopJsonUtils.parse(supplier);
    ...
    for (String version : versions) {
      assertThat(result.child(VERSIONS).child(version).child("dist").get("tarball", String.class),
          equalTo(format("%s/repository/%s/org/package/-/org-package-%s.tar.gz",
