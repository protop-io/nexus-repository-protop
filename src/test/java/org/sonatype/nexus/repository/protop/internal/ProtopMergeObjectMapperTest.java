...
public class ProtopMergeObjectMapperTest
    extends TestSupport
{
  private ProtopMergeObjectMapper underTest;

  @Before
  public void setUp() {
    underTest = new ProtopMergeObjectMapper();
  }
  ...
  private void verifyMergeVersionsTest(Function<List<InputStream>, NestedAttributesMap> function) {
    ...
    contents.add(new ByteArrayInputStream(ProtopJsonUtils.bytes(recessive)));
    contents.add(new ByteArrayInputStream(ProtopJsonUtils.bytes(dominant)));
  ...
  private NestedAttributesMap mergeInputStreamsAfterParse(final List<InputStream> inputStreams) {
    List<NestedAttributesMap> maps = inputStreams.stream()
        .map(this::parse)
        .collect(toList());
    return ProtopMetadataUtils.merge("test", maps);
  }

  private NestedAttributesMap parse(final InputStream inputStream) {
    try {
      return ProtopJsonUtils.parse(() -> inputStream);
    }
    catch (IOException e) {
      fail(e.getMessage());
    }
