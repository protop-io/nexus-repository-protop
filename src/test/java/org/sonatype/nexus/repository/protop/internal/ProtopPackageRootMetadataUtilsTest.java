...
public class ProtopPackageRootMetadataUtilsTest
    extends TestSupport
{
  @Rule
  ...
  @Test
  @SuppressWarnings("unchecked")
  public void createFullPackageMetadataUsingExtractPackageRootVersionUnlessEmptyWithAssetWithoutDistTagsLatest() throws Exception {
    when(packageRootBlob.getInputStream())
  ...
  private void createFullPackageMetadata(final BiFunction<String, String, String> function) throws URISyntaxException, IOException {
    createFullPackageMetadataImpl(function, "protop.json");
  }
  ...
  private void createFullPackageMetadataImpl(final BiFunction<String, String, String> function, String packageJsonFilename) throws URISyntaxException, IOException {
    try {
      ...
      File packageJsonFile = new File(ProtopPackageRootMetadataUtilsTest.class.getResource(packageJsonFilename).toURI());
      File archive = tempFolderRule.newFile();
      Map<String, Object> packageJson = new ProtopPackageParser()
          .parseProtopJson(() -> ArchiveUtils.pack(archive, packageJsonFile, "package/protop.json"));

      NestedAttributesMap packageMetadata = ProtopPackageRootMetadataUtils
  ...
  private static void assertPackageMetadata(final NestedAttributesMap packageMetadata) {
    ...
    assertThat(packageMetadata.get("name"), is("bar"));
    assertThat(packageMetadata.get("org"), is("@foo"));
    ...
    assertThat(versionData.get("name"), is("bar"));
    assertThat(versionData.get("org"), is("@foo"));
    ...
    assertThat(versionData.child("dist").get("tarball"),
        is("http://localhost:8080/repository/protop-hosted/@foo/bar/-/@foo-bar-1.5.3.tar.gz"));

    assertThat(versionData.child("engines").get("node"), is(">= 6.9.0"));
    assertThat(versionData.child("engines").get("protop"), is(">= 3.0.0"));
