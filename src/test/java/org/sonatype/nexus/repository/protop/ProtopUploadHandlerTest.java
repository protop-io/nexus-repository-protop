...
import org.sonatype.nexus.repository.protop.internal.ArchiveUtils;
...
public class ProtopUploadHandlerTest
    extends TestSupport
{
  private final String REPO_NAME = "protop-hosted";

  private ProtopUploadHandler underTest;
  ...
  public void setup() throws IOException, URISyntaxException {
    when(contentPermissionChecker.isPermitted(eq(REPO_NAME), eq(ProtopFormat.NAME), eq(BreadActions.EDIT), any()))
        .thenReturn(true);

    underTest = new ProtopUploadHandler(contentPermissionChecker, new SimpleVariableResolverAdapter(),
        new ProtopPackageParser(), emptySet());
    when(repository.facet(ProtopHostedFacet.class)).thenReturn(protopFacet);

    packageJson = new File(ProtopUploadHandlerTest.class.getResource("internal/protop.json").toURI());
    when(storageFacet.createTempBlob(any(PartPayload.class), any())).thenAnswer(invocation -> {
      TempBlob blob = mock(TempBlob.class);
      when(blob.get()).thenAnswer(i -> ArchiveUtils.pack(tempFolder.newFile(), packageJson, "package/protop.json"));
      return blob;
    });
  ...
  public void testGetDefinitionWithExtensionContributions() {
    ...
    underTest = new ProtopUploadHandler(contentPermissionChecker, new SimpleVariableResolverAdapter(),
        new ProtopPackageParser(), getDefinitionExtensions());
    UploadDefinition def = underTest.getDefinition();
  ...
  public void testHandle() throws IOException {
    ...
    assertThat(uploadResponse.getAssetPaths(), contains("foo/bar/-/bar/foo-bar-123.gz"));
    assertThat(uploadResponse.getComponentId().getValue(), is("nuId"));

    verify(contentPermissionChecker).isPermitted(eq(REPO_NAME), eq(ProtopFormat.NAME), eq(BreadActions.EDIT),
        captor.capture());
  ...
  public void testHandlePathWithDotInOrg() throws IOException, URISyntaxException {
    handlePath("package-path-with-dot-in-org.json");
  }
  ...
  @Test(expected = IllegalArgumentException.class)
  public void testHandlePathWithUnderscoreInOrg() throws IOException, URISyntaxException {
    handlePath("package-path-with-underscore-in-org.json");
  }
  ...
  private void handlePath(String jsonFile) throws IOException, URISyntaxException {
    packageJson = new File(ProtopUploadHandlerTest.class.getResource("internal/" + jsonFile).toURI());
