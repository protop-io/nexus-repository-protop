...
import static org.sonatype.nexus.repository.protop.internal.ProtopVersionComparator.versionComparator;

/**
 * protop utilities for manipulating protop metadata.
 *
 * @since 3.0
 */
public final class ProtopMetadataUtils {

    private static final Logger logger = LoggerFactory.getLogger(ProtopMetadataUtils.class);

    private ProtopMetadataUtils() {
        // nop
    }

    @VisibleForTesting
    static final DateTimeFormatter PROTOP_TIMESTAMP_FORMAT = ISODateTimeFormat.dateTime().withZone(DateTimeZone.UTC);
     ...
     */
    static Optional<NestedAttributesMap> selectVersionByTarballName(final NestedAttributesMap packageRoot, final String tarballName) {
        logger.info("raw tarball name: " + tarballName);
        String extractedTarballName = extractTarballName(tarballName);
        logger.info("extracted tarball name: " + extractedTarballName);
        ...
        for (String v : versions.keys()) {
            ...
            logger.info("version map " + version);
            String versionTarballUrl = version.child(DIST).get(TARBALL, String.class);
            if (!Strings.isNullOrEmpty(versionTarballUrl)) {
                logger.info("version tarball url " + versionTarballUrl);
                String comp = extractTarballName(versionTarballUrl);
                logger.info("comp " + comp);
                if (extractedTarballName.equals(comp)) {
    ...
    /**
     * Maintains the time fields of protop package root. Sets created time if it doesn't exist, updates the modified time.
     */
    static DateTime maintainTime(final NestedAttributesMap packageRoot) {
        ...
        final String nowString = PROTOP_TIMESTAMP_FORMAT.print(now);
    ...
    /**
     * Gets the last modified time field of protop package root. May return {@code null} if no such field.
     */
    @Nullable
    public static DateTime lastModified(final NestedAttributesMap packageRoot) {
        ...
        if (modified != null) {
            return PROTOP_TIMESTAMP_FORMAT.parseDateTime(modified);
        }
        return null;
    }
     ...
     * @param name              Name of the Asset/protop Package ID
     ...
     */
    public static String createRepositoryPath(final String org, final String name, final String version) {
        ProtopProjectId projectId = new ProtopProjectId(org, name);
        return String.format("%s/%s/-/%s-%s-%s.tar.gz", projectId.org(), projectId.name(), projectId.org(), projectId.name(), version);
    }

    /**
     * Shrinks the package root JSON object as required for protop search operations (basically shaves off version entry
     * values replacing them with tags. Package roots transformed like this must NOT be persisted back into storage
     * of Nexus, they are meant only by protop CLI downstream consumption.
     */
    @Nonnull
    ...
    /**
     * Overlays 2nd protop metadata object on top of 1st, with care about "shrunk" (version object-less) input. The {@code
     ...
     * {@code _rev} to make it un-editable by protop client. Package versions are NOT merged, they are replaced. Finally,
    ...
    public static NestedAttributesMap merge(final String key, final List<NestedAttributesMap> packages) {
        final NestedAttributesMap result = new NestedAttributesMap(key, Maps.newHashMap());
    ...
    public static NestedAttributesMap mergeContents(final List<Content> contents) throws IOException {
        ...
        return new ProtopMergeObjectMapper().merge(streams);
    }
    ...
    public static NestedAttributesMap parseContent(final Content content) throws IOException {
        return new ProtopMergeObjectMapper().read(content.openInputStream());
    }

    /**
     * Overlays two protop metadata objects, with care about "shrunk" (version document-less) input. The {@code recessive}
