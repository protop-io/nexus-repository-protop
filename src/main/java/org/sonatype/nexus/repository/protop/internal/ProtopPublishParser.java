...
/**
 * Parses incoming "protop publish" and "protop unpublish" JSON using the streaming API so that attachments are not read into
 ...
 * @since 3.4
 */
public class ProtopPublishParser extends ComponentSupport {
    ...
    private static final String protop_USER = "_protopUser";
     ...
     */
    public ProtopPublishParser(final JsonParser jsonParser,
    ...
    /**
     * Parses the {@code JsonParser}'s content into a {@code ProtopPublishOrDeleteRequest}. Temp blobs will be created if
     ...
     */
    public ProtopPublishRequest parse(@Nullable final String currentUserId) throws IOException {
        try {
            ...
            return new ProtopPublishRequest(packageRoot, tempBlobs);
    ...
    private void updateMaintainer(final NestedAttributesMap packageRoot, final String currentUserId) {
        ...
        if (distVersion != null && packageRoot.contains(VERSIONS_KEY)) {
            ...
            if (versionsMap.contains(distVersion)) {
                ...
                String protopUser = getProtopUser(versionToUpdate);
                if (isUserTokenBasedPublish(currentUserId, protopUser)) {
                    updateProtopUser(packageRoot, currentUserId);
                    updateMaintainerList(packageRoot, currentUserId);

                    updateProtopUser(versionToUpdate, currentUserId);
    ...
    private boolean isUserTokenBasedPublish(final String currentUserId, final String protopUser) {
        return protopUser != null && !currentUserId.equals(protopUser);
    }

    private String getProtopUser(final NestedAttributesMap packageEntry) {
        if (packageEntry.contains(protop_USER)) {
            NestedAttributesMap protopUser = packageEntry.child(protop_USER);
            return protopUser.get(NAME, String.class);
        }
        return null;
    }
    ...
    private String getDistTagVersion(final NestedAttributesMap packageRoot) {
        if (packageRoot.contains(DIST_TAGS)) {
            log.info("getting dist tag version");
            NestedAttributesMap distTag = packageRoot.child(DIST_TAGS);
            Iterator<Map.Entry<String, Object>> iterator = distTag.iterator();
            if (iterator.hasNext()) {
    ...
    private void updateProtopUser(final NestedAttributesMap packageEntry, final String currentUserId) {
        if (packageEntry.contains(protop_USER)) {
            NestedAttributesMap protopUser = packageEntry.child(protop_USER);
            protopUser.set(NAME, currentUserId);
        }
    }
