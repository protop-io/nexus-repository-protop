...
import static org.sonatype.nexus.repository.protop.internal.ProtopMetadataUtils.mergeContents;
import static org.sonatype.nexus.repository.protop.internal.ProtopMetadataUtils.parseContent;
...
/**
 * protop specific implementation of {@link GroupFacetImpl} allowing for {@link Cooperation}, merging and caching.
 */
@Named
@Exposed
public class ProtopGroupFacet extends GroupFacetImpl {
  ...
  @Inject
  public ProtopGroupFacet(
      @Named("${nexus.protop.mergeGroupMetadata:-true}") final boolean mergeMetadata,
  ...
  protected void configureCooperation(
      ...
      @Named("${nexus.protop.packageRoot.cooperation.enabled:-true}") final boolean cooperationEnabled,
      @Named("${nexus.protop.packageRoot.cooperation.majorTimeout:-0s}") final Time majorTimeout,
      @Named("${nexus.protop.packageRoot.cooperation.minorTimeout:-30s}") final Time minorTimeout,
      @Named("${nexus.protop.packageRoot.cooperation.threadsPerKey:-100}") final int threadsPerKey) {
  ...
  /**
   * Build the protop Package Root merging all the given responses into one. This method allows {@link Cooperation} to
  ...
  /**
   * Get {@link Content} wrapping the protop Package root for the {@link Context} of the current request
   ...
   */
  @Nullable
  public ProtopContent getFromCache(final Map<Repository, Response> responses, final Context context) throws IOException {
    Asset packageRootAsset = getPackageRootAssetFromCache(context);
    if (isNull(packageRootAsset)) {
      return null;
    }

    ProtopContent protopContent = toContent(getRepository(), packageRootAsset);
    protopContent.fieldMatchers(rewriteTarballUrlMatcher(getRepository(), packageRootAsset.name()));
    protopContent.missingBlobInputStreamSupplier(
        (missingBlobException) -> buildMergedPackageRootOnMissingBlob(responses, context, missingBlobException));

    return !isStale(protopContent) ? protopContent : null;
  }
   ...
   */
  @Nullable
  public ProtopContent getFromCache(final Context context) throws IOException {
    Asset packageRootAsset = getPackageRootAssetFromCache(context);
    if (isNull(packageRootAsset)) {
      return null;
    }
  ...
  protected Asset getPackageRootAssetFromCache(final Context context) throws IOException {
    ...
    return findPackageRootAsset(tx, tx.findBucket(getRepository()), projectId(matcherState(context)));
  }
  ...
  protected Content buildMergedPackageRoot(final Map<Repository, Response> responses, final Context context)
    ...
    if (shouldServeFirstResult(contents)) {
  ...
  protected Content saveToCache(final ProtopProjectId packageId, final NestedAttributesMap result) throws IOException {
  ...
  protected Asset savePackageRootToCache(final ProtopProjectId packageId, final NestedAttributesMap result) throws IOException {
    StorageTx tx = UnitOfWork.currentTx();
  ...
  protected void doInvalidate(final ProtopProjectId packageId) {
    StorageTx tx = UnitOfWork.currentTx();
  ...
  private Asset getAsset(final StorageTx tx, final ProtopProjectId packageId) {
    Bucket bucket = tx.findBucket(getRepository());
    Asset asset = findPackageRootAsset(tx, bucket, packageId);
  ...
  /**
   * @return True if we only have one result, false otherwise.
   */
  @VisibleForTesting
  protected boolean shouldServeFirstResult(final List<?> packages) {
  ...
  private void invalidatePackageRoot(final AssetEvent event) {
    ...
    try {
      doInvalidate(ProtopProjectId.parse(event.getAsset().name()));
    }
    finally {
      UnitOfWork.end();
    }
